...
class AsyncDynFuncHandler(BaseRpcHandler):
    ...
    def post(self, *args):
        ...
        function = self.GetFunc(method)
        if callable(function):
            args = list(args) + params
            d = defer.ensureDeferred(function(*args))
...
class TxRedis:
    ...
    @classmethod
    async def Acquire(cls, conn_obj, lock_key, lock_timeout=10, wait_func=None):
        acquired = False
        start = time.time()
        end_ts = start + lock_timeout
        while start < end_ts:
            temp = await cls.Eval(conn_obj, cls.ACQUIRE_SCRIPT, [lock_key], [lock_timeout, lock_key])
            if temp > 0:
                acquired = True
                break
            if wait_func:
                await wait_func(0.001)
            start = time.time()
        return acquired

    @classmethod
    async def FastAcquire(cls, conn_obj, lock_key, lock_timeout=10):
        temp = await cls.Eval(conn_obj, cls.ACQUIRE_SCRIPT, [lock_key], [lock_timeout, lock_key])
        return temp > 0

    @classmethod
    async def Release(cls, conn_obj, lock_key, locked):
        if locked:
            res = await cls.Eval(conn_obj, cls.RELEASE_SCRIPT, [lock_key], [lock_key])
            return res > 0
        else:
            return False
...
class String:
    ...
    # format "2C 12 34 FF"
    @classmethod
    def BinaryTextToBytes(cls, binary_text):
        import array
        items = binary_text.split(' ')
        binary_array = array.array('B')
        for item in items:
            if len(item) > 2 or len(item) < 1:
                raise ValueError("binary item must between 0-FF")
            binary_array.append(int(item, 16))
        return binary_array.tobytes()

    @classmethod
    def PrettyBinary(cls, bytes_obj, separator=" "):
        return separator.join(["{0:02X}".format(i) for i in bytes_obj])
...
class DateTime:
    ...
    @classmethod
    def toutc(cls, dt_obj, timezone_obj):
        return dt_obj - timezone_obj.utcoffset(dt_obj)
...
class Dict:
    ...
    @classmethod
    def DelNoneVals(cls, tbl, *key_args):
        for k in key_args:
            if k in tbl:
                if tbl[k] is None:
                    del tbl[k]
    ...
    @classmethod
    def HasVal(cls, tbl, key):
        if key in tbl:
            return tbl[key] is not None
        else:
            return False
...
class PageKiteProcess(protocol.ProcessProtocol):
